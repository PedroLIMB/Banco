...
class Banco():
    ...
    def verificar_login(self, email, senha):
        try:
            ...
            self.cursor = self.banco.cursor()
            self.cursor.execute(
                "SELECT * FROM banco WHERE email = ? AND senha = ?", (email, senha))
    ...
    def depositar(self, id_usuario, novo_saldo):
        try:
            self.banco = sqlite3.connect("bancoDip.db")
            self.cursor = self.banco.cursor()
            self.cursor.execute(
                "UPDATE banco SET saldo = ? WHERE id = ?", (novo_saldo, id_usuario))
            self.banco.commit()
            self.banco.close()
...
class Interface():
    ...
    def tela_login():
        def janela_login():
            ...
            direito = customtkinter.CTkFrame(
                master=janela_login, width=340, height=400, fg_color="#192042")
            ...
            label_logo = customtkinter.CTkLabel(
                master=direito, image=imagem, text="")
            ...
            login = customtkinter.CTkButton(
                janela_login, text="Login", command=lambda: Interface.fazer_login(email.get(), senha.get()))
            cadastrar = customtkinter.CTkButton(
                janela_login, text="Criar Conta", command=Interface.registrar_conta)
            ...
            email.configure(font=("Roboto", 16), width=300,
                            height=38, border_width=0)
            senha.place(x=40, y=200)
            senha.configure(font=("Roboto", 16), width=300,
                            height=38, border_width=0)

            login.place(x=40, y=250)
            login.configure(font=("Roboto", 16), width=300,
                            height=38, fg_color="#192042")

            cadastrar.place(x=40, y=290)
            cadastrar.configure(font=("Roboto", 14), width=300,
                                height=38, fg_color="#192042")
    ...
    def janela_principal(usuario):
        ...
        header = customtkinter.CTkFrame(
            master=janela_principal, width=1000, height=200, fg_color="#192042")
        header.place(x=0, y=0)
        frame_saldo = customtkinter.CTkFrame(
            master=janela_principal, width=300, height=400, fg_color="#192042")
        ...
        imagem = customtkinter.CTkImage(light_image=Image.open(logo))
        label_logo = customtkinter.CTkLabel(
            master=header, image=imagem, text="")
        titulo_saldo = customtkinter.CTkLabel(master=frame_saldo, text="Saldo")
        texto_saldo = customtkinter.CTkLabel(master=frame_saldo, text=f"Olá, {
                                             usuario[1]}!\nSeu saldo é: R${usuario[4]:.2f}")
        titulo_saque = customtkinter.CTkLabel(master=frame_saque, text="Saque")
        texto_saque = customtkinter.CTkLabel(
            master=frame_saque, text="Informe um valor para sacar:")
        entry_saque = customtkinter.CTkEntry(
            master=frame_saque, placeholder_text="Valor")
        botao_saque = customtkinter.CTkButton(
            master=frame_saque, text="Sacar", command=lambda: Interface.sacar(usuario[0], entry_saque.get()))

        titulo_deposito = customtkinter.CTkLabel(
            master=frame_deposito, text="Depósito")
        texto_deposito = customtkinter.CTkLabel(
            master=frame_deposito, text="Informe um valor para depositar:")
        entry_deposito = customtkinter.CTkEntry(
            master=frame_deposito, placeholder_text="Valor")
        botao_deposito = customtkinter.CTkButton(
            master=frame_deposito, text="Depositar", command=lambda: Interface.depositar(usuario[0], entry_deposito.get()))
        ...
        botao_saque.configure(font=("Roboto", 18), fg_color="#192042")
        botao_saque.place(x=35, y=250)
    ...
    def registrar_conta():
        ...
        if saldo_inicial_str:  # Verifica se o campo de saldo inicial não está vazio
            ...
            conta = Conta(None, titular, email_val, senha_val,
                          saldo_inicial, saldo_inicial * 4)
            banco = Banco()
...
esquerdo = customtkinter.CTkFrame(
    master=janela, width=340, height=400, fg_color="#192042")
...
confirmarsenha = customtkinter.CTkEntry(
    janela, placeholder_text="Confirmar Senha", show="*")
saldoinicial = customtkinter.CTkEntry(
    janela, placeholder_text="Insira um saldo inicial")
cadastrar = customtkinter.CTkButton(
    janela, text="Registrar-se", command=Interface.registrar_conta)
login = customtkinter.CTkButton(
    janela, text="Fazer Login", command=Interface.tela_login)
...
confirmarsenha.configure(font=("Roboto", 16), width=300,
                         height=38, border_width=0)
saldoinicial.place(x=370, y=260)
saldoinicial.configure(font=("Roboto", 16), width=300,
                       height=38, border_width=0)
cadastrar.place(x=370, y=310)
cadastrar.configure(font=("Roboto", 16), width=300,
                    height=38, fg_color="#192042")
